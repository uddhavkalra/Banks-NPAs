...
*log close
log using "Stata/Branches Cov/region_cov.log", replace
drop _all
...
// import excel "/Users/uddhav/Desktop/Thesis/state_data/state_nva.xlsx", sheet("2019-20") firstrow clear
...
replace banks = regexr(banks, "AUSTRALIA & NEW ZEALAND BANKING GROUP LTD.", "AUSTRALIA AND NEW ZEALAND BANKING GROUP LIMITED")
...
replace banks = regexr(banks, "FINCARE SMALL FINANCE BANK LIMITED ", "FINCARE SMALL FINANCE BANK LIMITED")

replace banks = regexr(banks, "IDFC FIRST BANK", "IDFC FIRST BANK LIMITED")

replace banks = regexr(banks, "INDUSTRIAL AND COMMERCIAL BANK OF CHINA LTD.", "INDUSTRIAL AND COMMERCIAL BANK OF CHINA")

replace banks = regexr(banks, "JAMMU AND KASHMIR BANK", "JAMMU & KASHMIR BANK LTD")

replace banks = regexr(banks, "JP MORGAN CHASE BANK NA", "JPMORGAN CHASE BANK NATIONAL ASSOCIATION")

replace banks = regexr(banks, "MASHREQ BANK", "MASHREQ BANK PSC")

replace banks = regexr(banks, "PT BANK MAYBANK INDONESIA TBK", "PT BANK MAYBANK INDONESIA TBK")

replace banks = regexr(banks, "UTKARSH SMALL FINANCE BANK", "UTKARSH SMALL FINANCE BANK LIMITED")
